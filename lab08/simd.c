...
long long int sum_simd(int vals[NUM_ELEMS]) {
    ...
    for(unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
        ...
        for(unsigned int i = 0; i < NUM_ELEMS / 4 * 4; i += 4) {
            ...
            __m128i and = _mm_and_si128(tmp, cmp);
            sum_vec = _mm_add_epi32(sum_vec, and);
        }
        int sum_tmp[4];
        _mm_storeu_si128((__m128i *) sum_tmp, sum_vec);
        int sum = sum_tmp[0] + sum_tmp[1] + sum_tmp[2] + sum_tmp[3];
        result += sum;
...
long long int sum_simd_unrolled(int vals[NUM_ELEMS]) {
    ...
    for(unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
        ...
        for(unsigned int i = last; i < NUM_ELEMS; i++) {
            if (vals[i] >= 128) {
                result += vals[i];
            }
        }
